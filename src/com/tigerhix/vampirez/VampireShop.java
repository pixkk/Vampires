        ...
        if (gamer.cash < cost) {
            ...
            this.plugin.getServer().getScheduler().runTaskLater(this.plugin, () -> {
                ...
                if (id == 0) {
                    ...
                    Location loc1 = evt.getWhoClicked().getLocation();
                    Location loc2 = gamer.getNearestSurvivor();
                    Vector direction = loc2.toVector().subtract(loc1.toVector());

                    loc1.getWorld().spawnParticle(Particle.VILLAGER_HAPPY, loc1, 100, direction.getX(), direction.getY(), direction.getZ(), 1);
                    ...
                    gamer.flameArrowID = VampireShop.this.plugin.getServer().getScheduler().scheduleSyncRepeatingTask(VampireShop.this.plugin, () -> {
                        for (final Location loc : places) {
                            player.getWorld().playEffect(loc, Effect.FIREWORK_SHOOT, 20);
                        }
                    }, 1L, 10L);
                    VampireShop.this.plugin.getServer().getScheduler().runTaskLater(VampireShop.this.plugin, new Runnable() {
                        @Override
                        public void run() {
                            VampireShop.this.plugin.getServer().getScheduler().cancelTask(gamer.flameArrowID);
                        }
